...
import org.CUCEI.HombreBomba.Objetos.Niveles.Nivel3;
...
public class EstadoNivel3 extends State {
	/**
	...
	public EstadoNivel3(ContenedorDelJuego juego) {
		this.juego = juego;
		// Añadimos mapa
		new Nivel3().crearMapa(listaObjetos);
			...
			if (listaObjetos.get(i).getNombre().equals("Jugador")) {
				...
				for (ObjetoDelJuego objeto : listaObjetos) {
					...
					if (objeto.getNombre().equals("Enemigo") && jugador.estaTocando(objeto)) {
						...
						juego.setState(new EstadoNivel3(juego));
					}
